...
const App: React.FC = () => {
  ...
  const [serviceForm, setServiceForm] = useState({
    title: '', category: 'tent', rate: '', unitType: 'Per Day', duration: '1 Day', itemsIncluded: '', contactNumber: '', _id: ''
  });
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}${endpoint}`, {
        ...
        body: JSON.stringify({
          ...
          itemsIncluded: Array.isArray(serviceForm.itemsIncluded) ? serviceForm.itemsIncluded : serviceForm.itemsIncluded.split(',').map(i => i.trim()),
          rate: Number(serviceForm.rate)
        })
      });
      if (res.ok) {
        alert(serviceForm._id ? "Service Updated!" : "Service Added!");
        setServiceForm({ title: '', category: 'tent', rate: '', unitType: 'Per Day', duration: '1 Day', itemsIncluded: '', contactNumber: '', _id: '' });
        fetchMyServices();
        setView('my-services');
      }
    } catch (e) { alert("Error saving service"); } finally { setLoading(false); }
  };
  ...
  const editService = (s: any) => {
    setServiceForm({ ...s, itemsIncluded: s.itemsIncluded.join(', ') });
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 pb-24 font-sans relative">
      ...
      <main className="p-6">
                  ...
                  {vendor.services?.map((s: any) => (
                    <div key={s._id} className="bg-gray-50 p-4 rounded-2xl border">
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {s.itemsIncluded?.map((item: string, i: number) => <span key={i} className="text-[9px] bg-white px-2 py-1 rounded border text-gray-500">{item}</span>)}
                      </div>
                      <button onClick={() => setBookingTarget(s)} className="w-full bg-blue-600 text-white py-2 rounded-xl text-[10px] font-black uppercase shadow-md">Book Now</button>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {view === 'vendor-dashboard' && (
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border">
            <h3 className="text-xs font-black text-purple-600 uppercase mb-4">{serviceForm._id ? 'Edit Service' : 'Add New Service'}</h3>
            <form onSubmit={handleAddOrUpdateService} className="space-y-3">
              <input placeholder="Service Title" className="w-full bg-gray-50 p-4 rounded-2xl outline-none border" value={serviceForm.title} onChange={e => setServiceForm({...serviceForm, title: e.target.value})} required />
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <input placeholder="Contact Number" type="tel" className="bg-gray-50 p-4 rounded-2xl outline-none border" value={serviceForm.contactNumber} onChange={e => setServiceForm({...serviceForm, contactNumber: e.target.value})} required />
              </div>
              <textarea placeholder="Items Included (comma separated)" className="w-full bg-gray-50 p-4 rounded-2xl outline-none border h-20" value={serviceForm.itemsIncluded} onChange={e => setServiceForm({...serviceForm, itemsIncluded: e.target.value})} required />
              <button className="w-full bg-purple-600 text-white py-4 rounded-2xl font-black text-[10px] shadow-lg">{serviceForm._id ? 'UPDATE SERVICE' : 'PUBLISH SERVICE'}</button>
            </form>
          </div>
        )}
        ...
        {view === 'my-services' && (
          <div className="space-y-4">
            <h2 className="font-black text-gray-800 text-lg mb-4">Manage My Services</h2>
            {myServices.map(s => (
              <div key={s._id} className="bg-white p-4 rounded-2xl shadow-sm border flex justify-between items-center">
                ...
                <button onClick={() => editService(s)} className="bg-purple-50 text-purple-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase">Edit</button>
              </div>
            ))}
          </div>
        )}
            ...
            {bookings.map(b => (
              <div key={b._id} className="bg-white p-5 rounded-2xl shadow-sm border border-l-4 border-l-blue-600">
                ...
                <div className="mt-4 pt-4 border-t flex justify-between items-center">
                  ...
                  <button className="text-[10px] font-black text-red-400 uppercase">Cancel</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 animate-slideUp">
            ...
            <form onSubmit={handleBooking} className="space-y-4">
              ...
              <textarea placeholder="Full Event Address" className="w-full bg-gray-50 p-4 rounded-2xl border outline-none h-24" onChange={e => setBookingForm({...bookingForm, address: e.target.value})} required />
              <div className="bg-blue-50 p-4 rounded-2xl flex justify-between items-center">
                <p className="text-xs font-bold text-blue-700">Service Rate</p>
                <p className="text-sm font-black text-blue-700">₹{bookingTarget.rate}/{bookingTarget.unitType}</p>
              </div>
              <button className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl">Confirm Booking</button>
            </form>
          </div>
        </div>
      )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md h-20 flex items-center justify-around border-t shadow-2xl z-50 rounded-t-[2.5rem] px-8">
        ...
        {user.role === 'vendor' && (
          <>
            <button onClick={() => setView('my-services')} className={`p-3 rounded-2xl ${view === 'my-services' ? 'text-purple-600 bg-purple-50' : 'text-gray-300'}`}><i className="fas fa-list-ul text-xl"></i></button>
            <button onClick={() => { setServiceForm({ title: '', category: 'tent', rate: '', unitType: 'Per Day', duration: '1 Day', itemsIncluded: '', contactNumber: '', _id: '' }); setView('vendor-dashboard'); }} className={`p-3 rounded-2xl ${view === 'vendor-dashboard' ? 'text-purple-600 bg-purple-50' : 'text-gray-300'}`}><i className="fas fa-plus-circle text-xl"></i></button>
          </>
        )}
